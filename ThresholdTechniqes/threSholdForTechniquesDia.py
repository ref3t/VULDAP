...
def checkCVEUsingAllTech():
    ...
    # for Threeshold in [0.5]:
        ...
        # print(f"{count}TechnqiueID: {key}\tTechnique: {value['TechnqiueName']}\tDescription: {value['TechnqiueDescription']}")
            ...
            if "." in key:
                techid2 = key.split(".")[0]
            trainAndTestSet = dataCve[dataCve['TechnqiueID'].str.startswith(techid2)]
    ...
    df.to_excel("FinalAttackMitreThreshold60.xlsx", index=False)
    # dfRes.to_excel("FinalAttackMitreThreshold60_2.xlsx", index=False)
