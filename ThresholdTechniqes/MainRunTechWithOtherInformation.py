...
def checkCVEUsingAllTech():
    ...
    tech_DataDictionary = readAllTechWithSubJustPositivesBalance()
    ...
    for infoData in informationData:
        ...
        for attack in attack_texts:
            ...
            # trainAndTestSet = dataCve.loc[dataCve['TechnqiueID'] == techid2]
            ...
            count = count +1
        if infoData == "Technique":
            df.to_excel(f"NewMapping/OutResults/TechniquesWithInfoResults/JustTechsExamplesNewwww.xlsx", index=False)
            dfRes.to_excel(f"NewMapping/OutResults/TechniquesWithInfoResults/JustTechsExamples2Newww.xlsx", index=False)
        elif infoData == "TechniqueWithSub":
