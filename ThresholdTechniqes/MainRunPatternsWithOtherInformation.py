...
def falseNegativeSUMAlltech2222(vul_data_array,CVEsAttack,capecID,arrayPositive ,arrayNegative,CvesNotAttack, Threeshold):
    ...
    # elif (arrayPositive) == 0  and countMapping > 0:
    #     AttackFN = 1 
    df = pd.concat([df, pd.DataFrame({'Threshold':[Threeshold*100],'TechID':[capecID],'TP': [arrayPositive], 'FP': [arrayNegative], 'FN': [(count2+count)], 'TN': [(trueNeativeResless+trueNeativeResNotRetrived)], 'AttackTP': [(AttackTP)], 'AttackTN': [(AttackTN)], 'AttackFP': [(AttackFP)], 'AttackFN': [(AttackFN)],'CountMapping':[(countMapping)]})], ignore_index=True)
...
def checkCVEUsingAllTech():
    ...
    # joined_list = [ CAPECName[i]+ " " + descriptions[i] for i in range(min(len(descriptions), len(techniquesName)))]
    ...
    for infoData in informationData:
        ...
        dfRes.to_excel(f"./resutls/AllCaoec2.xlsx", index=False)
