...
df = pd.DataFrame(columns=['Threshold','ID','TP', 'FP', 'FN', 'TN','AttackTP','AttackTN','AttackFP','AttackFN','CountMapping'])
    ...
    for infoData in informationData:
        ...
        for Threeshold in [0.60]:
            ...
            # #print(f"{count}TechnqiueID: {key}\tTechnique: {value['TechnqiueName']}\tDescription: {value['TechnqiueDescription']}")
                ...
                MappingCVEsVuldatNotForAttack = []
                MappingCVEsVuldatForAttack = []
                countlessThresholdandExist = 0
                countFN2 = 0
                countFN1 =0 
                countTN1 =0 
                countTN2 =0
                CVEsVuldatForAttackThresholdandExist = []
                for index in top_10_indices:
                    ...
                    if orgDescriptions.loc[index] is not None:
                        if not dataCve.loc[index]['CVEID'] in array:
                            ...
                            if float(vul_data.CVE_Smiliraty ) > Threeshold:
                                flagpositive = 1
                                for cve in trainAndTestSetCVEs:
                                    if vul_data.CVE_ID  == cve:
                                        arrayPositive.append(vul_data.CVE_ID )
                                        flagpositive = 0
                                        break
                                if flagpositive == 1:
                                    arrayNegative.append(vul_data.CVE_ID )
                            ###### MM
                            if vul_data.CVE_ID in trainAndTestSetCVEs:
                                MappingCVEsVuldatForAttack.append(vul_data.CVE_ID)
                            #### FN
                            if float(vul_data.CVE_Smiliraty) < Threeshold:
                                if vul_data.CVE_ID in trainAndTestSetCVEs:
                                    CVEsVuldatForAttackThresholdandExist.append(vul_data.CVE_ID)
                            # #print("*******************************************total CVEs from VULDAT less 50 And Exist In C***************************************************")
                            ###TN
                            if float(vul_data.CVE_Smiliraty) < Threeshold:
                                if vul_data.CVE_ID in CvesNotAttack:
                                    countTN1 = countTN1 +1
                ...
                countMapping = len(MappingCVEsVuldatForAttack)
                ####FN
                CVEsVuldatForAttackThresholdandExist = list(set(CVEsVuldatForAttackThresholdandExist))
                countFN1 = len(CVEsVuldatForAttackThresholdandExist)
                CVEsVuldatNotForAttack = []
                for item in trainAndTestSetCVEs:
                    ...
                    if flag == 0:
                        CVEsVuldatNotForAttack.append(item)
                CVEsVuldatNotForAttack = list(set(CVEsVuldatNotForAttack))
                countFN2 = len(CVEsVuldatNotForAttack)
                # #print("*******************************************not In VULDAT But In C ***************************************************")
                # #print(count2)
                # TN

                countTN2 = 0
                if len(CvesNotAttack) > 0:
                    for item in CvesNotAttack:
                        flag = 0 
                        for vuldat in vul_data_array:
                            if item == vuldat.CVE_ID:
                                flag = 1 
                ...
                AttackFP=0 
                AttackFN =0
                if ((len(arrayNegative)+len(arrayPositive))) > 0  and countMapping > 0:
                    AttackTP = 1 
                elif ((len(arrayNegative)+len(arrayPositive))) == 0  and countMapping == 0:
                    AttackTN = 1 
                elif ((len(arrayNegative)+len(arrayPositive))) > 0  and countMapping == 0:
                    AttackFP = 1 
                elif ((len(arrayNegative)+len(arrayPositive))) == 0  and countMapping > 0:
                ...
                # elif (arrayPositive) == 0  and countMapping > 0:
                #     AttackFN = 1 
                df = pd.concat([df, pd.DataFrame({'Threshold':[Threeshold*100],'ID':[key],'TP': [len(arrayPositive)], 'FP': [len(arrayNegative)], 'FN': [(countFN1+countFN2)], 'TN': [(countTN1+countTN2)], 'AttackTP': [(AttackTP)], 'AttackTN': [(AttackTN)], 'AttackFP': [(AttackFP)], 'AttackFN': [(AttackFN)],'CountMapping':[(countMapping)]})], ignore_index=True)
        ...
        df.to_excel(f"NewMapping/OutResults/Final{infoData}.xlsx", index=False)
        # dfRes.to_excel(f"NewMapping/OutResults/Final{infoData}2.xlsx", index=False)
