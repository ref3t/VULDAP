...
def readProceduresBalance():
    ...
    dict_items = list(data_dictP.items())

    # Randomly select 59 items
    random_items = random.sample(dict_items, 88)

    # Convert the selected items back to a dictionary
    data_dictP = dict(random_items)

    file_pathPositive = 'NewMapping/FinalResultSeperate/Procedures/ProceduresNegatives.xlsx'
    # Read the Excel file
    data = pd.read_excel(file_pathPositive, header=0, names=['ProceduresID', 'ProcedureDescription'])
    ...
    # Create a dictionary from the grouped data
    data_dictPN = grouped_data.set_index('ProceduresID').to_dict(orient='index')
    data_dictP.update(data_dictPN)
    # Return the dictionary if needed
    return data_dictP
...
def checkCVEUsingAllTech():
    ...
    for infoData in informationData:
        if infoData == "Technique":
            ...
            tech_dict = readProceduresBalance()
        ...
        for Threeshold in [0.60]:
            ...
            # #print(f"{count}TechnqiueID: {key}\tTechnique: {value['TechnqiueName']}\tDescription: {value['TechnqiueDescription']}")
                ...
                # count = count +1
        df.to_excel(f"NewMapping/OutResults/Final{infoData}Balanced2.xlsx", index=False)
